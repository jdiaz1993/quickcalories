...
export default function HistoryPage() {
  ...
  async function clearAll() {
    setClearing(true);
    setError(null);
    // Fetch all our estimate ids (RLS limits to own rows), then delete in batches
    ...
    <div className="min-h-screen px-4 py-6 bg-zinc-50 dark:bg-zinc-950">
      <div className="mx-auto max-w-md">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Latest {LIMIT} estimates (saved when signed in).
        </p>
        ...
        {loading ? (
          ...
          <p className="mt-6 text-sm text-zinc-500 dark:text-zinc-400">
            No estimates yet. Sign in and run estimates on the home page to see them here.
          </p>
        ) : (
          <ul className="mt-4 space-y-2">
            {list.map((row) => (
              <li
                key={row.id}
                className="flex items-center justify-between gap-2 rounded-lg border border-zinc-200 bg-white px-3 py-2.5 dark:border-zinc-700 dark:bg-zinc-900"
              >
